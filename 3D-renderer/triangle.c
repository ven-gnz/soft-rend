...
vec3_t barycentric_weights(vec2_t a, vec2_t b, vec2_t c, vec2_t p)
{
	...
	float area_parallelogram = (ac.x * ac.y - ac.y * ab.x); // AC x AB
...
void draw_texel(int x, int y, uint32_t* texture, vec2_t point_a, vec2_t point_b, vec2_t point_c, 
	float u0, float v0, float u1, float v1, float u2, float v2)
{
	...
	float interpolated_u = (u0) * a + (u1) * b + (u2) * g;
	float interpolated_v = (v0) * a + (v1) * b + (v2) * g;
	int tex_x = abs((int)(interpolated_u * texture_height));
	int tex_y = abs((int)(interpolated_v * texture_width));
	int fetching_index = ((texture_width * tex_y) + tex_x) % 4096;
	draw_pixel(x, y, texture[fetching_index]);
...
void draw_filled_triangle(int x0, int y0, int x1, int y1, int x2, int y2, uint32_t color)
{
		...
		int x2, int y2, float u2, float v2, uint32_t* texture)
	{
		...
		if (y2 - y0 != 0) inverse_slope1 = (float)(x2 - x1) / abs(y2 - y1);
		if (y2 - y0 != 0) inverse_slope2 = (float)(x2 - x0) / abs(y2 - y0);
